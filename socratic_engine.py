...
- No explanatory text outside JSON"""
        ...
        try:
            response = self.client.messages.create(
                ...
                system=self.SYSTEM_PROMPT.format(
                    context_json=json.dumps(current_context, indent=2, ensure_ascii=False),
            ...
            if result["type"] == "question":
                ...
                confirmation = f"Perfekt verstanden! ✓\n\n{result['summary']}\n\nStimmt das so?"
                conversation_history.append({
                    "role": "assistant",
                    "text": confirmation,
                    "timestamp": datetime.utcnow().isoformat()
    ...
    def _extract_context_from_history(
        ...
        filled_fields = sum([
            1 for field in self.GOOD_FIELDS 
            if current_context.get(field)
        ])
